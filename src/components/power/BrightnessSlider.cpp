...
BrightnessSlider::BrightnessSlider(GUdevDevice *device)
    : Gtk::Box(), device(device) {
  this->append(scale);
...
void BrightnessSlider::on_map() {
  ...
  BacklightService::get_instance().add_listener(
      "slider", [this](GUdevClient *client, const gchar *action,
                       GUdevDevice *udev_device, BacklightService *user_data) {
        if (!set_by_slider) {
          auto myName = g_udev_device_get_name(this->device);
          auto inName = g_udev_device_get_name(udev_device);

          if (strcmp(myName, inName) == 0) {
            auto max_brightness = g_udev_device_get_sysfs_attr_as_int(
                udev_device, "max_brightness");
            auto cur_brightness =
                g_udev_device_get_sysfs_attr_as_int(udev_device, "brightness");
...
void BrightnessSlider::on_scale_value_changed() {
  ...
  auto proxy =
      Gio::DBus::Proxy::create_sync(conn, SYSTEMD_DBUS_NAME, SYSTEMD_DBUS_PATH,
                                    SYSTEMD_DBUS_INTERFACE, nullptr, {});

  auto v =
      Glib::Variant<std::tuple<Glib::ustring, Glib::ustring, guint>>::create(
          std::make_tuple("backlight", g_udev_device_get_name(this->device),
                          255 * scale.get_value() / 100));
